...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    <head>
        ...
        <meta name="description" content="Masuk atau daftarkan usaha Anda di NADI.">
    ...
    <body class="min-h-screen bg-canvas font-sans text-ink antialiased">
        <main class="grid min-h-screen lg:grid-cols-[minmax(0,0.9fr)_minmax(32rem,1.1fr)]">
            <section class="relative hidden overflow-hidden bg-[#17201d] p-12 text-white lg:flex lg:flex-col lg:justify-between">
                ...
                <a href="{{ route('login') }}" class="relative inline-flex w-fit items-center gap-3 rounded-xl">
                ...
                <div class="relative max-w-xl">
                    <p class="text-xs font-semibold uppercase tracking-[0.18em] text-[#8ed3b6]">Ruang kerja pemilik usaha</p>
                    <h1 class="mt-5 text-4xl font-medium leading-tight tracking-[-0.04em] xl:text-5xl">Satu tempat untuk memahami usaha dan mengambil keputusan berikutnya.</h1>
                    <p class="mt-5 max-w-lg text-sm leading-7 text-white/55">Data pengguna dipisahkan dari data usaha, sehingga akun Anda siap mengelola lebih dari satu usaha di masa depan.</p>
                </div>

                <p class="relative text-xs text-white/35">Akses aman untuk pemilik usaha NADI.</p>
            </section>

            <section class="flex items-center justify-center px-5 py-10 sm:px-8 lg:px-12">
                <div class="w-full max-w-md">
                    <a href="{{ route('login') }}" class="mb-10 inline-flex items-center gap-3 rounded-xl lg:hidden">
