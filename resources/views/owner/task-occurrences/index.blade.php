    ...
    <div class="dashboard-sections">
        <header class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
                ...
                <p class="mt-2 text-sm leading-6 text-ink-muted">Pantau setiap occurrence karyawan tanpa mencampur riwayat antaranggota tim.</p>
            </div>
        ...
        <section class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4" aria-label="Ringkasan progres tugas">
            @foreach ([
        ...
        <section class="panel p-5 sm:p-6" aria-labelledby="task-filters-heading">
            <h2 id="task-filters-heading" class="text-base font-semibold text-ink">Filter occurrence</h2>
            <form method="GET" action="{{ route('task-occurrences.index') }}" class="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
                <input type="date" name="date" value="{{ $filters['date'] ?? '' }}" class="rounded-xl border border-line-soft bg-white px-3 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100">
                <select name="employee_id" class="rounded-xl border border-line-soft bg-white px-3 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100">
                ...
                <select name="status" class="rounded-xl border border-line-soft bg-white px-3 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100">
                ...
                <select name="priority" class="rounded-xl border border-line-soft bg-white px-3 py-2.5 text-sm outline-none focus:border-brand-500 focus:ring-4 focus:ring-brand-100">
                    <option value="">Semua prioritas</option>
                    @foreach ($priorities as $priority)
                        <option value="{{ $priority->value }}" @selected(($filters['priority'] ?? '') === $priority->value)>{{ $priority->label() }}</option>
                    @endforeach
                </select>
                <button type="submit" class="rounded-xl bg-brand-700 px-4 py-2.5 text-sm font-semibold text-white transition hover:bg-brand-800">Terapkan filter</button>
            </form>
        </section>
        ...
        <section class="panel overflow-hidden" aria-labelledby="occurrence-table-heading">
            <div class="flex items-center justify-between border-b border-line-soft px-5 py-4 sm:px-6">
                ...
                <span class="text-xs text-ink-faint">{{ $occurrences->total() }} occurrence</span>
            </div>
                    ...
                    <tbody class="divide-y divide-line-soft">
                        @forelse ($occurrences as $occurrence)
                            <tr class="align-top">
                                ...
                                <td class="px-5 py-4 text-ink-muted">{{ $occurrence->occurrence_date->format('d M Y') }}</td>
                                <td class="px-5 py-4 text-ink-muted">{{ $occurrence->due_at->format('d M Y, H:i') }}</td>
                                <td class="px-5 py-4">
                                    <span @class([
                                ...
                                <td class="max-w-56 px-5 py-4 text-xs leading-5 text-ink-muted">{{ $occurrence->employee_note ?: '—' }}</td>
                            </tr>
                        @empty
                            <tr><td colspan="6" class="px-6 py-12 text-center text-sm text-ink-muted">Tidak ada occurrence yang cocok dengan filter.</td></tr>
                        @endforelse
                    </tbody>
                </table>
            </div>
